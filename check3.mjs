...
const url='file://'+path.resolve('_therapy/index.html');
...
for(const v of [{n:'desktop',w:1440,h:900},{n:'tablet',w:768,h:1024},{n:'mobile',w:390,h:844,m:true}]){
  ...
  const r=await p.evaluate(()=>{
    ...
    return{
      ...
      hero:box('.hero'), h1:cs('.hero h1').fontSize+'/'+cs('.hero h1').fontWeight,
      h2:cs('.h2').fontSize, root:cs('body').fontSize,
      modCols:cs('.mod-grid').gridTemplateColumns.split(' ').length,
      statCols:(cs('.stats-grid').gridTemplateColumns.match(/px|fr/g)||[]).length,
      revCols:cs('.rev-grid').gridTemplateColumns.split(' ').length,
      sticky:cs('.parallax-sticky').position,
      burger:cs('.burger').display, navMenu:cs('.nav-menu').display,
      secSpace:box('.sec-space').h,
      btnRadius:cs('.btn-primary').borderRadius,
  ...
  await p.screenshot({path:`_therapy/shot-${v.n}.png`,fullPage:true});
  await c.close();
}
await b.close();
